...
async function loadBusinessTypes(
  ...
  const { rows } = await client.query<BusinessTypeMeta>(
...
async function loadVehicles(
  ...
  const { rows } = await client.query<VehicleMeta>(
...
export async function computeAutoBillingEntries(
  ...
  const staffBtMap = await loadBusinessTypes(client, staffBtIds);

  const clientIds = [...new Set(staffEntries.map((e) => e.client_id))];
  const autoTypes = await loadAutoBillingTypes(client, clientIds);
  ...
  const vehicleMap = await loadVehicles(client, vehicleIds);
  ...
  for (const [key, count] of surchargeCounts) {
    ...
    if (bt.billing_rule === "daily_once_per_area") {
      const billed = await alreadyBilledToday(client, ctx, clientId!, siteId!, bt.id);
      if (billed) continue;
