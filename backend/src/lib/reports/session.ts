...
/**
 * Direct port of Phase1 `frontend/src/lib/reports/session.ts`.
 *  - Validation order, error messages, and SQL preserved verbatim.
 *  - `AuthError(400|403, msg)` becomes NestJS Bad/Forbidden exceptions.
 *  - `PoolClient` becomes TypeORM `QueryRunner` (same `.query()` API).
 */
...
export function assertWorkDateNotFuture(workDate: string): void {
  if (workDate > jstWorkDate()) {
    throw new BadRequestException("未来の日付は選択できません");
  }
}

function assertCountForUnit(count: number, unit: InputUnit): void {
  if (count <= 0) {
    throw new BadRequestException("数量は0より大きく入力してください");
  }
  if (unit === "count" && !Number.isInteger(count)) {
    throw new BadRequestException("台数/回数は整数で入力してください");
  }
}
...
export function resolveSubmitStaffId(user: AuthedUser, staffId?: string): string {
  if (user.role === "admin") {
    if (!staffId) throw new BadRequestException("staff_id が必要です");
    return staffId;
  }
  if (user.role === "manager") {
    throw new ForbiddenException("マネージャは報告を作成できません");
  }
  if (!user.staffId) {
    throw new ForbiddenException("スタッフプロフィールが連携されていません");
  }
  return user.staffId;
}
...
export async function validateAndExpandSession(
  ...
  const blRows: Array<{ id: string }> = await qr.query(
    `SELECT id FROM business_lines WHERE id = $1 AND deleted_at IS NULL`,
    [input.business_line_id],
  );
  if (!blRows[0]) throw new BadRequestException("部門が見つかりません");

  if (user.role === "employee") {
    const ok = await employeeHasBusinessLine(qr, staffId, input.business_line_id);
    if (!ok) throw new ForbiddenException("この部門は担当外です");
  }
  ...
  for (const block of input.customer_blocks) {
    ...
    if (user.role === "employee") {
      const ok = await employeeHasClient(qr, staffId, block.client_id);
      if (!ok) throw new ForbiddenException("この顧客は担当外です");
    }

    const allowedSites = await visibleSiteIdsForClient(qr, user, block.client_id);
    const clientSites: Array<{ id: string }> = await qr.query(
    ...
    if (clientSites.length === 1) {
      ...
      throw new BadRequestException("拠点を選択してください");
    } else if (clientSites.length === 0) {
      throw new BadRequestException("顧客に拠点が登録されていません");
    }

    if (allowedSites !== null && siteId && !allowedSites.includes(siteId)) {
      throw new ForbiddenException("この拠点にはアクセスできません");
    }

    for (const entry of block.entries) {
      if (entry.count <= 0) continue;

      const btRows: BusinessTypeRules[] = await qr.query(
      ...
      const bt = btRows[0];
      if (!bt) throw new BadRequestException("業務内容が見つかりません");
      if (!bt.staff_enterable) {
        throw new BadRequestException(
          "この業務内容は自動加算のため選択できません",
        );
      }
      if (bt.client_id !== block.client_id) {
        throw new BadRequestException("業務内容が顧客と一致しません");
      }
      if (bt.business_line_id && bt.business_line_id !== input.business_line_id) {
        throw new BadRequestException("業務内容が部門と一致しません");
      }
      if (bt.site_id && bt.site_id !== siteId) {
        throw new BadRequestException("業務内容が選択した拠点と一致しません");
      }
      if (bt.site_id) {
        const siteOk: Array<{ ok: boolean }> = await qr.query(
          `SELECT EXISTS (
             SELECT 1 FROM sites
      ...
      if (bt.vehicle_select_mode && !entry.vehicle_id) {
        throw new BadRequestException(
          `${bt.name}：車両（またはステーション）を選択してください`,
        );
      }

      if (entry.vehicle_id) {
        const vehicleOk: Array<{ ok: boolean }> = await qr.query(
          `SELECT EXISTS (
        ...
        if (!vehicleOk[0]?.ok) {
          throw new BadRequestException("選択した車両が顧客と一致しません");
        }
      }
      ...
      if (memoErr) {
        throw new BadRequestException(`${bt.name}：${memoErr}`);
      }
  ...
  if (staffEntries.length === 0) {
    throw new BadRequestException("1件以上の業務を入力してください");
  }

  const autoEntries = await computeAutoBillingEntries(qr, staffEntries, {
